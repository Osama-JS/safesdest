...
class PlatformWalletExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths, WithTitle, WithEvents
{
    protected $data;
    ...
    {
        $lastRow = count($this->data) + 9; // 8 header rows + 1 data header
    ...
    private function addReportHeader($sheet)
    {
        ...
        $sheet->setCellValue('A6', 'ملخص سريع:');
    ...
    {
        $lastRow = count($this->data) + 10;

        // Summary title
        $sheet->setCellValue('A' . ($lastRow + 1), 'إحصائيات تفصيلية');
        $sheet->mergeCells('A' . ($lastRow + 1) . ':K' . ($lastRow + 1));

        $sheet->setCellValue('A' . ($lastRow + 2), 'عدد المهام الكلي: ' . $this->statistics['total_tasks']);
        $sheet->setCellValue('C' . ($lastRow + 2), 'عدد المهام المحصلة: ' . $this->statistics['paid_tasks']);
        $sheet->setCellValue('F' . ($lastRow + 2), 'عدد المهام المعلقة: ' . $this->statistics['pending_tasks']);

        $sheet->setCellValue('A' . ($lastRow + 3), 'عمولات (Dynamic): ' . number_format($this->statistics['dynamic_commissions'], 2) . ' SAR');
        $sheet->setCellValue('C' . ($lastRow + 3), 'عمولات (Manual): ' . number_format($this->statistics['manual_commissions'], 2) . ' SAR');

        // Style summary
        $sheet->getStyle('A' . ($lastRow + 1) . ':K' . ($lastRow + 3))->applyFromArray([
            'font' => ['bold' => true],
            'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]
        ]);
