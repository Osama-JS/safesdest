...
class PaymentController extends Controller
{
  ...
  public function initiatePaymentClearance(Request $request)
  {
    $validator = Validator::make($request->all(), [
      'id' => 'nullable|exists:customs_clearance,id',
      'payment_method' => 'required|in:credit,banking,wallet',
    ]);
    if ($validator->fails()) {
      return response()->json(['status' => 0, 'error' => $validator->errors()->toArray()]);
    }
    $task = Customs_Clearance::find($request->input('id'));

    DB::beginTransaction();
    try {
      if (in_array($request->payment_method, ['credit'])) {
        if ($request->payment_method === 'credit') {
          $amount = $task->total_price;
          $payment_paid = 'all';
        } else {
        ...
        if ($checkoutData && isset($checkoutData['result']['code']) && $checkoutData['result']['code'] == '000.200.100') {
          ...
          $transaction = $user->transactionsClearance()->create([
            ...
            'payment_type' => 'credit',
            'reference_id' => $task->id,
            'note' => 'دفع رسوم خدمة تخليص جمركي',
            'checkout_id' => $checkoutData['id']
          ]);
        ...
        if ($task->customer_id == null) {
          return response()->json([
            'status' => 2,
            'error' => __('You can not pay using this method! you do not hav a wallet'),
          ]);
        }
        $amount = $task->total_price;
        ...
        $data = [
          'amount'              => $amount,
          'description'         => 'Pay the fee for custome clearance #' . $task->id,
          'transaction_type'    => 'debit',
          'wallet_id'           => $wallet->id,
          'maturity_time'       => Carbon::now()->copy()->addDays(3),
        ];
        ...
        $transaction = $task->customer->transactionsClearance()->create([
          'amount' => $amount,
          'status' => 'completed',
          'type' => 'delivery',
          'payment_type' => 'wallet',
