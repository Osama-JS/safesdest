/**
 * Team Tasks Report JavaScript
 * SafeDests Platform Reports
 */
class TeamTasksReport {
  constructor() {
    this.availableColumns = {
      ...
      team_info: { name: 'Team Information', required: true },
    ...
    this.selectedColumns = ['task_id', 'total_price', 'team_info'];
  ...
  init() {
    console.log('TeamTasksReport: Starting initialization...');
    ...
    console.log('TeamTasksReport: Initialization complete');
  }
  ...
  initializeDateRangePicker() {
    if (typeof moment === 'undefined' || typeof $.fn.daterangepicker === 'undefined') return;
  ...
  buildSummarySection(summary) {
    return `
      <div class="mt-3 p-3 bg-light rounded">
        ...
        <div class="row">
          <div class="col-md-4"><strong>Total Tasks:</strong> ${summary.total_tasks}</div>
          <div class="col-md-4"><strong>Total Amount:</strong> ${parseFloat(summary.total_amount).toLocaleString()} SAR</div>
          <div class="col-md-4"><strong>Average Amount:</strong> ${parseFloat(summary.average_amount).toLocaleString()} SAR</div>
        </div>
      </div>
    `;
  }
  ...
  getFormData() {
    return {
      ...
      include_payment_summary: $('#include_payment_summary').is(':checked') ? 1 : 0,
      include_commission_info: $('#include_commission_info').is(':checked') ? 1 : 0
    };
  }

  validateForm() {
    if (!$('#team_ids').val()?.length) {
      this.showError('Please select at least one team');
      return false;
    }
  ...
  savePreferences() {
    localStorage.setItem('teamTasksReportColumns', JSON.stringify(this.selectedColumns));
  }
  loadSavedPreferences() {
    const saved = localStorage.getItem('teamTasksReportColumns');
    if (saved) {
      try {
...
window.TeamTasksReport = TeamTasksReport;
