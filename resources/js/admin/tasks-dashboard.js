...
$(function () {
  ...
  function loadTasks(page = 1, search = '', retries = 3) {
    ...
    $.ajax({
      url: baseUrl + 'admin/dashboard/tasks/data',
      type: 'GET',
        ...
        if (retries > 0) {
          ...
          $('.body-container-block').unblock({
            onUnblock: function () {
              ...
              $('#task-running-container, #task-complete-container').html(
        ...
        <div class="card p-3 shadow-sm task-card" data-task-id="${task.id}">
          <div class="d-flex justify-content-between">
            <div class="d-flex align-items-center">
              ...
              <div class="px-3">
                <h6 class="mb-1">${task.name} #${task.id}</h6>
    ...
    $.ajax({
      ...
      `;
        ...
        $('#close-task-details').on('click', function () {
          ...
          // 1. إزالة النافذة المنبثقة من الخريطة (إن وُجدت)
          if (window.taskPopupInstance) {
            window.taskPopupInstance.remove();
            window.taskPopupInstance = null;
          }

          // 2. إعادة موازنة عرض الخريطة لتشمل كل المهام
          const source = taskMapInstance.getSource('tasks');
          if (source && source._data && source._data.features.length > 0) {
            const bounds = new mapboxgl.LngLatBounds();
            source._data.features.forEach(f => bounds.extend(f.geometry.coordinates));

            taskMapInstance.fitBounds(bounds, {
              padding: 50,
              maxZoom: 11
            });
          }
