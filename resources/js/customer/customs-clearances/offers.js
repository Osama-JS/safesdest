...
$(function () {
  ...
  function loadOffers() {
    ...
    $.ajax({
      url: baseUrl + 'customer/customs-clearances/ads/offers/show',
      type: 'GET',
      data: { id: adId },
  ...
  $(document).on('click', '#accept-task', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: ` ${baseUrl}customer/customs-clearances/ads/offers/accept/task/${id}`,
          type: 'GET',
          success: function (response) {
            if (response.status === 1) {
              showAlert('success', response.success, 10000, true);
              showAlert('info', 'you well redirect to the customs clearance page', 20000, true);
              setTimeout(() => {
                location.href(baseUrl + 'customer/customs-clearances/ads');
              }, 2000);
            } else {
