    ...
    <script>
        ...
        function initializeReport() {
            if (checkLibrariesLoaded()) {
                if (typeof DriverTasksReport !== 'undefined') {
                    console.log('All libraries loaded. Initializing DriverTasksReport...');
                    new DriverTasksReport();
                } else {
                    console.error('DriverTasksReport class not found');
                }
            } else {
    ...
    <div class="card mb-4">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-2">
                ...
                {{ __('Platform Reports') }} | {{ __('Driver Tasks Report') }}
            </h5>
            <p>{{ __('Generate detailed reports for driver tasks with customizable filters') }}
            </p>
        </div>
    </div>
                        ...
                        <form id="reportForm">
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3">
                                    <label for="driver_ids" class="form-label">{{ __('Select Drivers') }} <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select  filter-select" id="driver_ids" name="driver_ids[]" multiple
                                        required>
                                        @foreach ($drivers as $driver)
                                            <option value="{{ $driver->id }}">{{ $driver->name }} - {{ $driver->phone }}
                                                @if ($driver->team)
                                                    ({{ $driver->team->name }})
                                                @endif
                                            </option>
