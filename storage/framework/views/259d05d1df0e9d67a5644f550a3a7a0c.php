...
<?php $__env->startSection('title', __('Investment Wallet - :name', ['name' => $user->name])); ?>
...
<?php $__env->startSection('vendor-script'); ?>
    <?php echo app('Illuminate\Foundation\Vite')(['resources/assets/vendor/libs/moment/moment.js', 'resources/assets/vendor/libs/datatables-bs5/datatables-bootstrap5.js', 'resources/assets/vendor/libs/sweetalert2/sweetalert2.js']); ?>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('page-script'); ?>
    <script>
        const investorId = <?php echo e($user->id); ?>;
        const transactionsDataUrl = '<?php echo route('admin.investors.invest-wallet.getTransactions', ['userId' => $user->id, 'from_date' => request('from_date'), 'to_date' => request('to_date')]); ?>';
        const addTransactionUrl = '<?php echo e(route('admin.investors.invest-wallet.addTransaction')); ?>';
        const convertTransactionUrl = '<?php echo e(url('admin/investors/invest-wallet/transaction/convert')); ?>';
        const cancelInvestmentUrl = '<?php echo e(url('admin/investors/invest-wallet/transaction/cancel-investment')); ?>';
        const isOsama = <?php echo e(auth()->user()->email === 'osama.samomy@gmail.com' ? 'true' : 'false'); ?>;
    </script>
    <?php echo app('Illuminate\Foundation\Vite')(['resources/js/admin/investor-wallets.js']); ?>
    <script>
        ...
        window.checkFunding = function() {
            ...
            $.ajax({
                url: baseUrl + 'admin/investors/<?php echo e($user->id); ?>/invest-wallet/check-funding',
                type: 'GET',
        ...
        window.fixAnomaly = function(anomalyType, fixAction, taskId, transactionId) {
            ...
            $.ajax({
                url: baseUrl + 'admin/investors/<?php echo e($user->id); ?>/invest-wallet/fix-funding',
                type: 'POST',
        ...
        window.openManualSettlementModal = function() {
            ...
            $.ajax({
                url: baseUrl + 'admin/investors/<?php echo e($user->id); ?>/invest-wallet/unsettled-tasks',
                type: 'GET',
                success: function(response) {
                    if (response.status === 1) {
                        let html = '';
        ...
        window.addEventListener('load', function () {
            $(document).on('click', '#submitManualSettlementBtn', function() {
                ...
                $.ajax({
                    url: baseUrl + 'admin/investors/<?php echo e($user->id); ?>/invest-wallet/manual-settlement',
            ...
            window.executeCapitalWithdrawal = function(id, amount) {
                Swal.fire({
                    title: '<?php echo e(__("Confirm Capital Return Disbursement")); ?>',
                    html: `
                        <div class="text-start mb-3">
                            <p class="mb-2">${'<?php echo e(__("Are you sure you want to disburse and return :amount SAR to the investor? This amount will be debited immediately from the investment wallet.", ["amount" => "###AMOUNT###"])); ?>'.replace('###AMOUNT###', '<strong class="text-danger">' + amount + '</strong>')}</p>
                        </div>
                        <div class="mb-3 text-start">
                            <label class="form-label text-danger fw-bold mb-1" for="swal_admin_password">
                                <i class="ti ti-lock me-1"></i><?php echo e(__("Admin Password to Confirm Disbursement *")); ?>

                            </label>
                            <input type="password" id="swal_admin_password" class="form-control border-danger" placeholder="<?php echo e(__('Enter your password to confirm debit')); ?>" autocomplete="new-password">
                        </div>
                        <div class="mb-2 text-start">
                            <label class="form-label fw-semibold mb-1" for="swal_admin_notes">
                                <?php echo e(__("Transfer Reference / Notes (Optional)")); ?>

                            </label>
                            <input type="text" id="swal_admin_notes" class="form-control" placeholder="<?php echo e(__('Transfer Reference / Notes (Optional)')); ?>">
                        </div>
                    `,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '<?php echo e(__("Confirm and Disburse")); ?>',
                    ...
                    preConfirm: () => {
                        ...
                        if (!password || !password.trim()) {
                            Swal.showValidationMessage('<?php echo e(__("Admin password is required")); ?>');
                            return false;
                        }
                        ...
                        return $.ajax({
                            ...
                            if (response.status !== 1) {
                                throw new Error(response.error || '<?php echo e(__("Failed to process request")); ?>');
                            }
                            return response;
                        }).catch(error => {
                            Swal.showValidationMessage(error.message || '<?php echo e(__("Failed to process request")); ?>');
                        });
                    }
                }).then((result) => {
                    if (result.isConfirmed) {
...
<?php $__env->startSection('content'); ?>
        ...
        <div class="col-12">
            <div class="card bg-primary text-white border-0 shadow-sm">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        <h4 id="investorName" class="mb-1 text-white fw-bold"><?php echo e($user->name); ?></h4>
                        <p class="mb-0 opacity-75"><?php echo e($user->email); ?></p>
                        <span class="badge bg-white text-primary mt-2"><?php echo e(__('Investor #:id', ['id' => $user->id])); ?></span>
                    </div>
                    <div class="ms-auto d-none d-md-block">
                        <a href="<?php echo e(route('admin.investors.index')); ?>" class="btn btn-outline-white btn-sm">
                            <i class="ti ti-arrow-right me-1"></i> <?php echo e(__('Back to Investors List')); ?>
    ...
    <?php if(isset($dueDisbursementRequests) && $dueDisbursementRequests->isNotEmpty()): ?>
        <?php $__currentLoopData = $dueDisbursementRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dueReq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="alert alert-danger d-flex align-items-center mb-4 shadow-sm border-start border-4 border-danger" role="alert">
            ...
            <div class="flex-grow-1">
                <h5 class="alert-heading fw-bold mb-1 text-danger">
                    <i class="ti ti-alert-circle me-1"></i><?php echo e(__('Alert: Capital Return Disbursement is Due!')); ?>

                </h5>
                <p class="mb-0">
                    <?php echo e(__('There is a capital withdrawal request of :amount SAR for investor :name, scheduled for disbursement on :date (the 3-month period has ended).', [
            ...
            <button type="button" class="btn btn-danger btn-sm shadow ms-3 px-3 py-2" onclick="executeCapitalWithdrawal(<?php echo e($dueReq->id); ?>, '<?php echo e(number_format($dueReq->amount, 2)); ?>')">
                <i class="ti ti-check me-1"></i><?php echo e(__('Execute Disbursement Now')); ?>
    ...
    <?php if(isset($pendingDisbursementRequests) && $pendingDisbursementRequests->isNotEmpty()): ?>
        <?php $__currentLoopData = $pendingDisbursementRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pendReq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="alert alert-warning d-flex align-items-center mb-4 shadow-sm border-start border-4 border-warning" role="alert">
            ...
            <div class="flex-grow-1">
                <h6 class="alert-heading fw-bold mb-1 text-warning">
                    <i class="ti ti-clock me-1"></i><?php echo e(__('Approved Capital Withdrawal Request (Scheduled for Disbursement):')); ?>

                </h6>
                <p class="mb-0 small">
                    <?php echo e(__('Capital withdrawal request #:id of :amount SAR has been approved. Scheduled disbursement date: :date (Remaining: :remaining).', [
                        'id'        => $pendReq->id,
            ...
            <button type="button" class="btn btn-outline-warning btn-sm ms-3" onclick="executeCapitalWithdrawal(<?php echo e($pendReq->id); ?>, '<?php echo e(number_format($pendReq->amount, 2)); ?>')">
                <?php echo e(__('Early Disbursement')); ?>
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    <form action="<?php echo e(route('admin.investors.invest-wallet', $user->id)); ?>" method="GET" class="row g-3 align-items-end">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4 d-flex gap-2">
                            ...
                            <a href="<?php echo e(route('admin.investors.invest-wallet', $user->id)); ?>" class="btn btn-outline-secondary">
                                إعادة ضبط
                            </a>
                            <a href="<?php echo e(route('admin.investors.invest-wallet.export', ['userId' => $user->id, 'from_date' => request('from_date'), 'to_date' => request('to_date')])); ?>" class="btn btn-success ms-auto">
            ...
            <div class="card border-0 shadow-sm overflow-hidden">
                <div class="card-body position-relative">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <span class="text-muted fw-medium"><?php echo e(__('Current Available Balance Label')); ?></span>
                    </div>
                    <div class="d-flex align-items-baseline">
                        <h3 class="mb-0 fw-bold text-success"><?php echo e(number_format($balance, 2)); ?></h3>
                        <span class="ms-2 text-muted"><?php echo e(__('SAR')); ?></span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm overflow-hidden">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <span class="text-muted fw-medium"><?php echo e(__('Total Capital Label')); ?></span>
                    </div>
                    <div class="d-flex align-items-baseline">
                        <h3 class="mb-0 fw-bold text-info"><?php echo e(number_format($credit, 2)); ?></h3>
                        <span class="ms-2 text-muted"><?php echo e(__('SAR')); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm overflow-hidden">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <span class="text-muted fw-medium"><?php echo e(__('Total Investment Returns')); ?></span>
                    </div>
                    <div class="d-flex align-items-baseline">
                        <h3 class="mb-0 fw-bold text-warning"><?php echo e(number_format($returned_capital, 2)); ?></h3>
                        <span class="ms-2 text-muted"><?php echo e(__('SAR')); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm overflow-hidden">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <span class="text-muted fw-medium"><?php echo e(__('Total withdrawals funding')); ?></span>
                    </div>
                    <div class="d-flex align-items-baseline">
                        <h3 class="mb-0 fw-bold text-danger"><?php echo e(number_format($debit, 2)); ?></h3>
                        <span class="ms-2 text-muted"><?php echo e(__('SAR')); ?></span>
                    </div>
                </div>
            </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header border-bottom d-flex justify-content-between align-items-center py-3">
            <h5 class="card-title mb-0 fw-bold"><?php echo e(__('Wallet transactions log investor')); ?></h5>
            <div>
                <?php if($user->activeInvestmentContract): ?>
                <a href="<?php echo e(route('admin.user-wallets.tasks-funding', $user->id)); ?>" class="btn btn-secondary me-2">
                    <i class="ti ti-cash me-1"></i> تمويل المهام
                </a>
                <?php endif; ?>
                <?php if(auth()->user()->email === 'osama.samomy@gmail.com'): ?>
                ...
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('manual_investment_settlement')): ?>
                ...
                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#transactionModal">
                    <i class="ti ti-plus me-1"></i> <?php echo e(__('Add new operation')); ?>
            ...
            <table id="investorTransactionsTable" class="datatables-transactions table">
                <thead>
                    <tr>
                        ...
                        <th><?php echo e(__('Source Type')); ?></th>
                        <th><?php echo e(__('Statement description')); ?></th>
    ...
    <?php if(isset($capitalWithdrawalRequests) && $capitalWithdrawalRequests->isNotEmpty()): ?>
    <div class="card border-0 shadow-sm mt-4 mb-4">
        <div class="card-header border-bottom d-flex justify-content-between align-items-center py-3 bg-label-secondary">
            <h5 class="card-title mb-0 fw-bold text-heading">
                <i class="ti ti-arrow-up-right text-danger me-2"></i><?php echo e(__('Capital Withdrawal Requests from Wallet')); ?>

            </h5>
            <span class="badge bg-label-danger fs-6"><?php echo e($capitalWithdrawalRequests->count()); ?> <?php echo e(__('registered requests')); ?></span>
        </div>
            ...
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th><?php echo e(__('Scheduled Disbursement Date (after 3 months)')); ?></th>
                        <th><?php echo e(__('Remaining Duration')); ?></th>
                ...
                <tbody>
                    <?php $__currentLoopData = $capitalWithdrawalRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wReq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr>
                        <td><strong>#<?php echo e($wReq->id); ?></strong></td>
                        <td class="fw-bold text-danger"><?php echo e(number_format($wReq->amount, 2)); ?> <?php echo e(__('SAR')); ?></td>
                        <td><?php echo e($wReq->request_date->format('Y-m-d H:i')); ?></td>
                        <td>
                            <span class="badge bg-label-info">
                                <i class="ti ti-calendar me-1"></i><?php echo e($wReq->scheduled_disbursement_date ? $wReq->scheduled_disbursement_date->format('Y-m-d') : '—'); ?>

                            </span>
                        </td>
                        <td>
                            <?php if($wReq->status === 'completed'): ?>
                                <span class="badge bg-label-success"><?php echo e(__('Disbursed')); ?></span>
                            <?php elseif($wReq->status === 'rejected'): ?>
                                <span class="badge bg-label-danger"><?php echo e(__('Rejected')); ?></span>
                            <?php elseif($wReq->is_due_for_disbursement): ?>
                                <span class="badge bg-danger animate__animated animate__pulse animate__infinite"><?php echo e(__('Due for Disbursement')); ?></span>
                            <?php else: ?>
                                <span class="text-muted fw-medium"><?php echo e($wReq->remaining_duration_human); ?></span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo $wReq->status_badge; ?></td>
                        <td class="small">
                            <?php if($wReq->investor_notes): ?>
                                <div><strong class="text-muted"><?php echo e(__('Investor:')); ?></strong> <?php echo e($wReq->investor_notes); ?></div>
                            <?php endif; ?>
                            <?php if($wReq->admin_notes): ?>
                                <div><strong class="text-dark"><?php echo e(__('Admin:')); ?></strong> <?php echo e($wReq->admin_notes); ?></div>
                            <?php endif; ?>
                            <?php if(!$wReq->investor_notes && !$wReq->admin_notes): ?>
                                <span class="text-muted">—</span>
                            <?php endif; ?>
                        </td>
                        <td>
                            <div class="d-flex align-items-center gap-1">
                                <?php if($wReq->status === 'pending'): ?>
                                    <button type="button" class="btn btn-sm btn-success" title="<?php echo e(__('Approve Request and Schedule Disbursement')); ?>" onclick="approveCapitalWithdrawal(<?php echo e($wReq->id); ?>, '<?php echo e(number_format($wReq->amount, 2)); ?>')">
                                        <i class="ti ti-check me-1"></i><?php echo e(__('Approve')); ?>

                                    </button>
                                    <button type="button" class="btn btn-sm btn-danger" title="<?php echo e(__('Reject')); ?>" onclick="rejectCapitalWithdrawal(<?php echo e($wReq->id); ?>)">
                                        <i class="ti ti-x me-1"></i><?php echo e(__('Reject')); ?>

                                    </button>
                                <?php elseif($wReq->status === 'approved'): ?>
                                    <button type="button" class="btn btn-sm btn-primary shadow-sm" title="<?php echo e(__('Execute Disbursement')); ?>" onclick="executeCapitalWithdrawal(<?php echo e($wReq->id); ?>, '<?php echo e(number_format($wReq->amount, 2)); ?>')">
                                        <i class="ti ti-cash me-1"></i><?php echo e(__('Execute Disbursement')); ?>

                                    </button>
                                <?php elseif($wReq->status === 'completed'): ?>
                                    <span class="badge bg-label-success small"><i class="ti ti-check-double me-1"></i><?php echo e(__('Executed')); ?></span>
                                <?php else: ?>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="modalTitle"><?php echo e(__('Add New Transaction')); ?></h5>
                ...
                <form id="transactionForm" class="pt-0" onsubmit="return false" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <!-- Transaction Type -->
                        <div class="mb-4">
                            <label class="form-label d-block fw-bold">* <?php echo e(__('Transaction Type')); ?></label>
                            <div class="btn-group w-100" role="group">
                                ...
                                <label class="btn btn-outline-success py-2" for="typeCredit">
                                    <i class="ti ti-circle-plus me-1"></i> <?php echo e(__('Capital Deposit (Credit)')); ?>

                                </label>

                                <input type="radio" class="btn-check" name="type" id="typeDebit" value="debit">
                                <label class="btn btn-outline-danger py-2" for="typeDebit">
                                    <i class="ti ti-circle-minus me-1"></i> <?php echo e(__('Withdrawal / Debit from Capital (Debit)')); ?>
                        ...
                        <div id="creditAlert" class="alert alert-primary d-flex align-items-center mb-4" role="alert">
                            ...
                            <span><?php echo e(__('This transaction will increase the investor available balance.')); ?></span>
                        </div>
                        ...
                        <div id="debitAlert" class="alert alert-danger d-none mb-4" role="alert">
                            <div class="d-flex align-items-center mb-1">
                                ...
                                <strong class="text-danger"><?php echo e(__('Important Security Warning:')); ?></strong>
                            </div>
                            <span class="small" style="line-height: 1.5;">
                                <?php echo e(__('Debit Capital Warning Notice')); ?>
                        ...
                        <!-- Amount -->
                        <div class="mb-4">
                            <label class="form-label fw-semibold" for="amount">* <?php echo e(__('Amount')); ?></label>
                            <div class="input-group">
                                <input type="number" name="amount" id="amount" class="form-control"
                                    placeholder="<?php echo e(__('Enter the amount')); ?>" step="0.01" min="0.01" required>
                        ...
                        <!-- Description -->
                        <div class="mb-4">
                            <label class="form-label fw-semibold" for="description">* <?php echo e(__('Description / Notes')); ?></label>
                            <textarea name="description" id="description" class="form-control" rows="3"
                                placeholder="<?php echo e(__('Enter transaction details...')); ?>" required></textarea>
                        </div>
                        ...
                        <div id="adminPasswordGroup" class="mb-4 d-none">
                            <label class="form-label text-danger fw-bold" for="admin_password">
                                <i class="ti ti-lock me-1"></i><?php echo e(__('* Admin Password to Confirm Debit')); ?>

                            </label>
                            <input type="password" name="admin_password" id="admin_password" class="form-control border-danger"
                                placeholder="<?php echo e(__('Enter your password to confirm debit')); ?>" autocomplete="new-password">
                            <div class="form-text text-muted"><?php echo e(__('Admin password required note for debit')); ?></div>
                        </div>

                        <!-- Attachment -->
                        <div class="mb-0">
                            <label class="form-label fw-semibold" for="attachment"><?php echo e(__('Upload Receipt / Attachment')); ?></label>
                            <input type="file" name="attachment" id="attachment" class="form-control"
                                accept=".jpg,.jpeg,.png,.pdf">
                            <div class="form-text text-muted mt-1">
                                <small><?php echo e(__('Supported formats: JPEG, PNG, PDF. Max size: 2MB')); ?></small>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" id="modalSubmitBtn" class="btn btn-primary btn-submit">
                            ...
                            <?php echo e(__('Submit Transaction')); ?>
    ...
    <div class="modal fade" id="convertTransactionModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold"><?php echo e(__('Convert deposit modal title')); ?></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p class="mb-3"><?php echo __('Convert deposit confirm'); ?></p>
                    <p class="text-muted"><?php echo e(__('Convert deposit note')); ?></p>
                    ...
                    <div class="mb-3">
                        <label class="form-label" for="convertTransactionPassword"><?php echo e(__('Password required for conversion')); ?></label>
                        <input type="text" name="fakeusernameremembered" style="display:none" autocomplete="username"><input type="password" id="convertTransactionPassword" class="form-control"
                            placeholder="<?php echo e(__('Enter password')); ?>" autocomplete="new-password" required>
                        <div class="form-text text-muted"><?php echo e(__('Password required for conversion')); ?></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal"><?php echo e(__('Cancel')); ?></button>
                    <button type="button" id="confirmConvertTransaction" class="btn btn-warning"><?php echo e(__('Yes convert now')); ?></button>
                </div>
            </div>
        </div>
    </div>
...
<?php echo $__env->make('layouts/layoutMaster', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\safedestssss\resources\views/admin/investors/wallets/invest.blade.php ENDPATH**/ ?>